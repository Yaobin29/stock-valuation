...
st.markdown(f"### 🧠 行业对比判断：<span style='font-weight:bold;color:{'green' if industry_judge=='低估' else 'red'}'>{industry_judge}</span>", unsafe_allow_html=True)

# 获取新闻情绪评分
sentiment_score = fetch_news_sentiment(row["name_en"])
...
st.markdown(f"### 💬 情绪面分析判断：<span style='font-weight:bold;'>{sentiment_judge}</span>", unsafe_allow_html=True)
...
sent_score = 0.5 if sentiment_judge == "中性" else (0 if sentiment_judge == "正面" else 1)
...
# 高亮显示最终判断
color = "green" if final_judge == "低估" else "red" if final_judge == "高估" else "orange"
st.markdown(f"""
<div style="border: 2px solid {color}; padding: 10px; border-radius: 8px; background-color:#f9f9f9;">
<h4>🧮 综合估值判断（技术60% + 情绪40%） × 模型50% + 行业50% ：
<span style='color:{color}'>{final_judge}</span></h4>
</div>
